...
```{r Exercise 1.7}
target <- c("Duster 360", "Lincoln Continental", "Maserati Bora")
match(target, rownames(mtcars))
match("hp", colnames(mtcars))
mtcars[c(7,6,31), 4]
```
...
```{r}
write.csv(clinic, "/home1/nickxu/490_cluster/analysis_data/brca_clinical_data.csv", row.names=FALSE)
```

To read the csv back into your environment as a dataframe:

```{r}
clinic <- read.csv("/home1/nickxu/490_cluster/analysis_data/brca_clinical_data.csv")
```
...
y <- suppressWarnings(as.numeric(clinic$lymph_nodes_examined_count))
grp <- factor(clinic$gender)
cols <- c(FEMALE = "red", MALE = "blue")
plot(x, y, pch = 19,col = cols[grp], xlab = "Age at diagnosis (years)",
     ylab = "Lymph nodes examined (count)", main = "Age vs Lymph Nodes Examined")
```
...
boxplot(age_at_diagnosis ~ gender, data = clinic,
        ...
        xlab = "Gender", ylab = "Age at diagnosis (years)",
...
```{r}
under30_over60_mask <- !is.na(clinic$age_at_diagnosis) & clinic$age_at_diagnosis<30 | clinic$age_at_diagnosis>60
```
...
cleaned_clinic <- clinic[!age_NA_mask, , drop = FALSE]
```
...
```{r}
dd <- suppressWarnings(as.numeric(cleaned_clinic$death_days_to))
lc <- suppressWarnings(as.numeric(cleaned_clinic$last_contact_days_to))
cleaned_clinic$survival_time <- ifelse(!is.na(dd), dd, lc)
```
...
Probability of being alive after time t 
...
The p-value is 0.2. Since it is greater than 0.05, the data is not statistically significant. The data does not provide strong evidence that different age groups have different survival distributions.
